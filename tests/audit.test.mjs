...
export async function run() {
  ...
  // ── находки консилиума, которые легко вернуть обратно ───────────────────
  const прочитать = (путь) => readFileSync(join(root, путь), 'utf8')

  // Записать давление можно без кнопки на приборе: кнопок две, людей в семье
  // сколько угодно, и жена с детьми видели вместо формы тупик.
  const экранДавления = прочитать('src/App.tsx')
  check(
    'форма записи давления не спрятана за кнопкой прибора',
    !/deviceUser === null \? \(\s*<Banner/.test(экранДавления),
  )
  check('и запись без кнопки уходит с нулём', /user: user \?\? 0/.test(прочитать('src/ui/Entry.tsx')))

  // Нажатие по напоминанию ведёт туда, куда позвали.
  check(
    'напоминание об измерении открывает давление, а не таблетки',
    /about === 'measure' \? 'bp'/.test(экранДавления),
  )
  check('а об анализе — анализы', /about === 'lab' \? 'labs'/.test(экранДавления))

  // Вход в анализы не зависит от того, есть ли уже измерения.
  const дверьВАнализы = экранДавления.indexOf("onOpen={() => setTab('labs')}")
  const ветка = экранДавления.indexOf('bpAll.length + glucoseAll.length === 0')
  check('вход в анализы стоит до ветки «есть измерения»', дверьВАнализы > 0 && дверьВАнализы < ветка,
    `${дверьВАнализы} против ${ветка}`)

  // Удаление записи живёт в правке, а не в четырёх точках от карандаша.
  for (const экран of ['src/ui/Readings.tsx', 'src/ui/Glucose.tsx']) {
    check(`${экран.split('/').pop()}: корзины в строке нет`, !/TrashIcon/.test(прочитать(экран)))
  }
  check('а в правке есть', /Удалить запись/.test(прочитать('src/ui/EditRow.tsx')))

  // Поле ввода в тёмной теме светлее карточки, а не темнее.
  const стили = прочитать('src/app.css')
  check('у поля свой фон, а не цвет страницы', /--dark--field:/.test(стили))
  check('и барабан не набран приглушённым', /\.wheel__item \{[\s\S]*?color: var\(--text-secondary\)/.test(стили))
