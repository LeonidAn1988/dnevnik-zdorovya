...
export const SCREENS = [
  ...
  { name: 'Настройки — человек', tool: 'Настройки', open: ['Пользователи', 'Я'] },
...
async function main() {
  ...
  for (const профиль of ПРОФИЛИ) {
    ...
    const context = await browser.newContext({
      ...
      colorScheme: theme,
      /*
       * Сенсорный экран, а не мышь.
       *
       * Без этого `matchMedia('(pointer: coarse)')` отвечает «нет», и
       * приложение показывает настольный контрол ввода `− 120 +` вместо
       * барабана. Полтора года снимки и эталоны Percy ловили виджет, которого
       * на телефоне не видит никто, а тот, которым пользуются все, не попал ни
       * в один снимок.
       *
       * Первый прогон после этой правки станет новым эталоном Percy — иначе и
       * быть не может, картинки меняются по делу.
       */
      hasTouch: true,
    })
