...
export type Node =
   ...
   *
   * `edit: 'left'` — открыть сразу с полем остатка: из «Заканчивается» и
   * «Купить» приходят именно за этим, и лишнее касание по кнопке «Поправить
   * остаток» на пустом месте здесь ни к чему.
   */
...
/**
 * Разделы из шапки: ключ, подпись и якорь для гайд-курса.
 *
 * Живут здесь, а не в `App.tsx`, по одной причине: рассказ о них — в курсе
 * (`logic/tour.ts`), и пока эти два списка были порознь, они разошлись молча.
 * Кнопок стало четыре, а курс продолжал говорить «Три кнопки сверху», и
 * заметил это владелец, а не тест. Теперь подписи одни на оба места.
 *
 * Значки остаются в `App.tsx`: это разметка, и в модуле без DOM ей не место.
 */
export const TOOL_ITEMS: { key: string; label: string; tour?: string }[] = [
  { key: 'sync', label: 'Прибор' },
  { key: 'report', label: 'Отчёт' },
  { key: 'settings', label: 'Настройки' },
  // Якорь только здесь: на эту кнопку показывает последний шаг базового курса.
  // Остальным трём якоря не нужны — их шаг обводит весь ряд разом, а мёртвая
  // разметка это та же мёртвая строка кода, и сторож ниже её находит.
  { key: 'guide', label: 'Справка', tour: 'tool-guide' },
]

/** Подписи кнопок шапки через запятую — для текста шага курса. */
export function toolLabels(): string[] {
  return TOOL_ITEMS.map((item) => item.label)
}
