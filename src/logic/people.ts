...
/**
 * Курсы приёма выбранного человека — с явным ответом на «человека нет».
 *
 * Пустой список, а не все курсы всех. Раньше отбор стоял прямо в экране и при
 * неизвестном человеке отдавал **всё**: «Обзор» под одним именем показывал
 * чужие приёмы — с красным «!» и кнопкой «Принял» в двух касаниях. Найдено на
 * живом дневнике 23 сентября 2026 (`BACKLOG.md` §24и).
 *
 * Запасной путь «показать всех» был неверен и без этого случая. `activePersonOf`
 * возвращает `null`, только когда список людей пуст, — а при пустом списке «все
 * курсы» это не разумное умолчание, а показ чужого под своим именем. Пустая
 * карточка «Сегодня» на секунду человека не обманет; чужие таблетки обманут.
 *
 * Тип `T` вместо `Regimen`: экран приёма работает со склейкой коробки и курса
 * (`Dosing`), у которой своего `id` нет, а `person` есть.
 */
export function intakesOfPerson<T extends { person: string }>(intakes: T[], personId: string | null): T[] {
  return personId === null ? [] : intakes.filter((item) => item.person === personId)
}
